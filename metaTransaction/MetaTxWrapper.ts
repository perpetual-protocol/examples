...
import { NetworkName, getNetworkConfig } from "../networks"
import logger, { LogMetadataSet } from "../logger"
import { trackSendTxToFuncRequest, trackSendTxToFuncSent } from "src/segment"

import { BiconomyError } from "src/util/error"
import { LedgerProvider } from "../ethers-provider/LedgerProvider"
import { MetaTxGateway } from "src/types/contracts"
import { TypedData } from "src/constant/type"
import { TypedDataUtils } from "ethers-eip712"
import { getRSVBySignature } from "../utils"
import { hexlify } from "ethers/lib/utils"
import metadata from "src/metadata/ethereum.json"
import { permittableTokenAbi } from "../abi"
...
export class MetaTxWrapper {
    ...
    async executeMetaTxGateway(functionSignature: string, gasLimit: BigNumber): Promise<string> {
        ...
        const network = getNetworkConfig(metadata.layers.layer1.network as NetworkName)
    ...
    async permit(spender: string): Promise<string> {
        ...
        const network = getNetworkConfig(metadata.layers.layer2.network as NetworkName)
        ...
        const typedData = this.getTypedData("Permit", name, network.chainId, this.contract.address, message)
